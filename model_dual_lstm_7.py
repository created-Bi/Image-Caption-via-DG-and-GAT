...
class CaptionGeneratin(nn.Module):
    def __init__(self, vocab_size, triple_size, dis_size, lstm_hidden_dim, attention_dim, embedd_dim, encoder_dim=2048,
        ...
        self.init_h1_cap = nn.Linear(embedd_dim, lstm_hidden_dim)  # linear layer to find initial hidden state of LSTMCell
        self.init_c1_cap = nn.Linear(embedd_dim, lstm_hidden_dim)
        self.init_h2_cap = nn.Linear(encoder_dim, lstm_hidden_dim)  # linear layer to find initial hidden state of LSTMCell
        self.init_c2_cap = nn.Linear(encoder_dim, lstm_hidden_dim)
    ...
    def init_hidden_state(self, all_image_feature, related_words_embeddings):
        ...
        h2 = self.init_h2_cap(all_image_feature)  # (batch_size, lstm_hidden_dim)
        c2 = self.init_c2_cap(all_image_feature)  # (batch_size, lstm_hidden_dim)
        h1 = self.init_h1_cap(related_words_embeddings)  # (batch_size, lstm_hidden_dim)
        c1 = self.init_c1_cap(related_words_embeddings)  # (batch_size, lstm_hidden_dim)
        return h1, c1, h2, c2
...
class DecoderModel(nn.Module):
    ...
    def forward(self, out_root, out_caption, encoded_captions, caption_lengths, encoded_triples, layer):
        ...
        left_graph_embeddings = torch.add(left_root_node_embeddings, left_distance_embedding_from_ck)  # batch_size, iter_times, embedding_dim
        right_graph_embeddings = torch.add(right_root_node_embeddings, right_distance_embedding_from_ck)  # batch_size, iter_times, embedding_dim
        '''why we use self-attention'''
        left_graph_embeddings = self.graph_left_attention(q=left_graph_embeddings, k=right_graph_embeddings, v=right_graph_embeddings, mask=None)  # batch_size, iter_times, embedding_dim
        right_graph_embeddings = self.graph_right_attention(q=right_graph_embeddings, k=left_graph_embeddings, v=left_graph_embeddings, mask=None)  # batch_size, iter_times, embedding_dim
        ...
        caption_predictions, ds_predictions, end_score, alphas = self.generation(out_caption, encoded_triples,  decode_lengths, captions_embedding, graph_embeddings)
        ...
        hidden_vector = captions_embedding.mean(dim=1).clone()  # batch_size, max_length, hidden_dim
        root_loss = (torch.LongTensor([1]).to(device) - torch.cosine_similarity(root_node_embedding, hidden_vector, eps=1e-8)).mean()
