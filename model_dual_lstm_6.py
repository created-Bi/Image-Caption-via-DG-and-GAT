...
class CaptionGeneratin(nn.Module):
    def __init__(self, vocab_size, triple_size, dis_size, lstm_hidden_dim, attention_dim, embedd_dim, encoder_dim=2048,
        ...
        self.decode_step = nn.LSTMCell(embedd_dim + embedd_dim + encoder_dim, lstm_hidden_dim)
        self.decode_ds_step = nn.LSTMCell(embedd_dim + embedd_dim, lstm_hidden_dim)
        self.init_h1_cap = nn.Linear(encoder_dim, lstm_hidden_dim)  # linear layer to find initial hidden state of LSTMCell
        self.init_c1_cap = nn.Linear(encoder_dim, lstm_hidden_dim)
        self.init_h2_cap = nn.Linear(embedd_dim, lstm_hidden_dim)  # linear layer to find initial hidden state of LSTMCell
    ...
    def forward(self, encoder_out, encoded_triples, decode_lengths, captions_embedding, related_words_embeddings):#, relations_embeddings
        '''
        ...
        h_similiar = torch.zeros(batch_size, max(decode_lengths), self.embedd_dim).to(device)
        ...
        for t in range(max(decode_lengths)):  # (0-25)
            ...
            score = self.graph_weight(torch.bmm(related_words_embeddings[:batch_size_t], h1[:batch_size_t].unsqueeze(-1)).squeeze(-1))  # batch_size, iter_times
            rws = (related_words_embeddings[:batch_size_t] * score.unsqueeze(-1)).sum(dim=1)  # batch_size, embedding_dim
            ...
            h_similiar[:batch_size_t, t, :] = h2
            end_score[:batch_size_t, t] = torch.exp(preds[:, 7463] + triple_preds[:, 5282])
        return caption_predictions, ds_predictions, end_score, alphas, h_similiar
...
class Graph_Encoding(nn.Module):
    ...
    def forward(self, root_node_idx, mlp_out, words_embeddings, dis_embeddings):
        ...
        distance_embedding_from_ck[:, 0, :] = dis_embeddings(torch.LongTensor(batch_size * [10]).to(device))  # batch_size, embedding_dim
        ...
        for i in range(self.iter_times - 1):
            ...
            root_input = torch.cat([self.ROOTupdimension(root_nodes_embeddings), self.IMGdedimension(mlp_out)], dim=1)  # batch_size, 1024 + 512 = 1536
            ...
            p_related_words = self.relatedWordsClassifier(related_input).squeeze()  # batch_size, vocab_size  (logSoftmax)
            ...
            p_related_words = p_related_words.unsqueeze(0)
        ...
        return root_node_idxs, distance_embedding_from_ck
...
class DecoderModel(nn.Module):
    def __init__(self, words_embeddings_from_graph, dis_embeddings_from_graph,
        ...
        self.graph_left_encoding = Graph_Encoding(vocab_size,dis_size, encoder_dim, embedd_dim, dropout, hidden_size, iter_times, self.adjacent_words_embedding)
        self.graph_right_encoding = Graph_Encoding(vocab_size, dis_size, encoder_dim, embedd_dim, dropout, hidden_size, iter_times, self.adjacent_words_embedding)
        self.graph_self_attention = selfAttention(embedd_dim)
    ...
    def forward(self, out_root, out_caption, encoded_captions, caption_lengths, encoded_triples, layer):
        ...
        left_root_node_idxs, left_distance_embedding_from_ck = self.graph_left_encoding(root_node_idx, out_root, self.words_embedding, self.dis_embedding)
        right_root_node_idxs, right_distance_embedding_from_ck = self.graph_right_encoding(root_node_idx, out_root, self.words_embedding, self.dis_embedding)
        ...
        left_words_embeddings = torch.add(left_root_node_embeddings, left_distance_embedding_from_ck)  # batch_size, iter_times, embedding_dim
        right_words_embeddings = torch.add(right_root_node_embeddings, right_distance_embedding_from_ck)  # batch_size, iter_times, embedding_dim

        '''why we use self-attention'''
        graph_embeddings = self.graph_self_attention(q=left_words_embeddings, k=right_words_embeddings, v=right_words_embeddings, mask=None)  # batch_size, iter_times, embedding_dim

        # for i in range(layer):
        #     graph_embeddings = self.graph_self_attention(q=graph_embeddings, k=graph_embeddings, v=graph_embeddings, mask=None)
        #print('Generation processing...')
        caption_predictions, ds_predictions, end_score, alphas, h_similiar = self.generation(out_caption, encoded_triples, decode_lengths, captions_embedding, graph_embeddings)

        end_score = pack_padded_sequence(end_score, [l - 1 for l in decode_lengths], batch_first=True).data
        end_score = end_score.mean() if end_score.mean() < 10 else torch.FloatTensor([10]).to(device)

        root_node_embedding = self.words_embedding(root_node_idx)  # batch_size, embedd_dim
        hidden_vector = h_similiar.sum(dim=1)  # batch_size, hidden_dim
        root_loss = torch.FloatTensor([1]).to(device)-(torch.cosine_similarity(root_node_embedding, hidden_vector, eps=1e-6).mean())
